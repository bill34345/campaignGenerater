...
const connectionResponseSchema = z.object({ status: z.literal("ok") }).strict();
...
function parseJsonContent<T>(schema: z.ZodType<T>, content: unknown) {
  const text =
    typeof content === "string"
      ...
      : Array.isArray(content)
        ? content
            ...
            .join("\n")
        : "";
  return schema.parse(JSON.parse(normalizeJsonCandidate(text)));
}

function parseRawJsonContent(content: unknown) {
  const text =
    typeof content === "string"
      ? content
      : Array.isArray(content)
...
function parseFactExtractionContent(content: unknown) {
  const parsed = parseRawJsonContent(content);
        ...
        outcomeOptions:
          outcomeOptions.length > 0
            ...
            : [
                locale === "zh"
                  ? "取得新的线索或主动权"
                  : "Gain a new lead or the initiative",
              ],
        };
    });
...
function buildQuestPrompt(workingContext: TownQuestContext, questRequest: QuestRequest) {
  ...
  const labels = isChinese
    ? {
        ...
        onlyJson: "只返回结构化 JSON，不要输出 schema 之外的散文。",
    ...
    : {
        ...
        onlyJson: "Return structured JSON only. Do not return prose outside the schema.",
...
export const openAIChatAdapter: LlmProviderAdapter = {
    ...
    const response = await client.chat.completions.create({
      ...
      response_format: {
        ...
        json_schema: {
          name: "quest_draft",
          strict: true,
          schema: z.toJSONSchema(questGenerationSchema),
        },
      },
    });
    ...
    for (const chunk of chunks) {
      const response = await client.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You extract campaign canon facts and return only structured JSON.",
          },
          {
            role: "user",
            content: buildFactPrompt(chunk),
          },
        ],
        response_format: {
          type: "json_schema",
          json_schema: {
            ...
            strict: true,
            schema: z.toJSONSchema(factExtractionResponseSchema),
          },
        },
      });

      const normalized = parseFactExtractionContent(
        response.choices[0]?.message?.content,
      );
    ...
    const response = await client.chat.completions.create({
      ...
      response_format: {
        ...
        json_schema: {
          ...
          strict: true,
          schema: z.toJSONSchema(connectionResponseSchema),
        },
      },
    });
    parseJsonContent(connectionResponseSchema, response.choices[0]?.message?.content);
